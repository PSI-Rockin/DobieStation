...
{
    Logger::log(Logger::FPU, "MTC1: %d, $%08X\n", index, value);
    gpr[index].u = value;
}
...
{
    Logger::log(Logger::FPU, "CTC1: $%08X (%d)\n", value, index);
    switch (index)
    {
        case 31:
...
void Cop1::cvt_s_w(int dest, int source)
{
    ...
    Logger::log(Logger::FPU, "CVT_S_W: %f(%d)\n", gpr[dest].f, dest);
}
...
void Cop1::cvt_w_s(int dest, int source)
{
    ...
    Logger::log(Logger::FPU, "CVT_W_S: $%08X(%d)\n", gpr[dest].u, dest);
}
...
void Cop1::add_s(int dest, int reg1, int reg2)
{
    ...
    gpr[dest].f = op1 + op2;
    Logger::log(Logger::FPU, "add.s: %f(%d) + %f(%d) = %f(%d)\n", op1, reg1, op2, reg2, gpr[dest].f, dest);
}
...
void Cop1::sub_s(int dest, int reg1, int reg2)
{
    ...
    float op2 = convert(gpr[reg2].u);
    gpr[dest].f = op1 - op2;
    Logger::log(Logger::FPU, "sub.s: %f(%d) - %f(%d) = %f(%d)\n", op1, reg1, op2, reg2, gpr[dest].f, dest);
}
...
void Cop1::mul_s(int dest, int reg1, int reg2)
{
    ...
    float op2 = convert(gpr[reg2].u);
    gpr[dest].f = op1 * op2;
    Logger::log(Logger::FPU, "mul.s: %f(%d) * %f(%d) = %f(%d)\n", op1, reg1, op2, reg2, gpr[dest].f, dest);
}
...
void Cop1::adda_s(int reg1, int reg2)
{
    ...
    Logger::log(Logger::FPU, "adda.s: %f + %f = %f\n", op1, op2, accumulator.f);
}
...
void Cop1::suba_s(int reg1, int reg2)
{
    ...
    float op2 = convert(gpr[reg2].u);
    accumulator.f = op1 - op2;
    Logger::log(Logger::FPU, "suba.s: %f - %f = %f\n", op1, op2, accumulator.f);
}
...
void Cop1::mula_s(int reg1, int reg2)
{
    ...
    float op2 = convert(gpr[reg2].u);
    accumulator.f = op1 * op2;
    Logger::log(Logger::FPU, "mula.s: %f * %f = %f\n", op1, op2, accumulator.f);
}
...
void Cop1::madd_s(int dest, int reg1, int reg2)
{
    ...
    Logger::log(Logger::FPU, "madd.s: %f + %f * %f = %f\n", acc, op1, op2, gpr[dest].f);
}
...
void Cop1::msub_s(int dest, int reg1, int reg2)
{
    ...
    float acc = convert(accumulator.u);
    gpr[dest].f = acc - (op1 * op2);
    Logger::log(Logger::FPU, "msub.s: %f - %f * %f = %f\n", acc, op1, op2, gpr[dest].f);
}
...
void Cop1::madda_s(int reg1, int reg2)
{
    ...
    Logger::log(Logger::FPU, "madda.s: %f + (%f * %f) = %f", acc, op1, op2, accumulator.f);
}
