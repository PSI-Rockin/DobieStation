...
{
    VectorUnit& vu0 = info.cpu->get_VU0();
    vu0.decoder.reset();

    uint8_t op = instruction & 0x3F;
    info.interpreter_fn = &cop2_special_exec;

    // Decoder switch/case
    // TODO: Latencies and other info (maybe another switch/case afterwards?)
    switch (op)
    {
        case 0x00:
        case 0x01:
...
void EmotionInterpreter::cop2_special_exec(EmotionEngine& cpu, uint32_t instruction)
{
    /**
  * FIXME: IMPORTANT!
  ...
  */
    ...
    switch (op)
    {
        ...
        case 0x3E:
        case 0x3F:
            cop2_special2_exec(vu0, instruction);
            break;
        default:
            unknown_op("cop2 special", instruction, op);
    }
...
void EmotionInterpreter::cop2_special2_exec(VectorUnit &vu0, uint32_t instruction)
{
    uint16_t op = (instruction & 0x3) | ((instruction >> 4) & 0x7C);
    switch (op)
    {
